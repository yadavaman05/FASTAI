...
const navItems = [
    ...
    {to: '/ai/remove-resume', label: 'Remove Resume', Icon: FileText},
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
            {navItems.map(({to, label, Icon})=>(
                <NavLink key={to} to={to} end= {to === '/ai'} onClick={()=>
                    setSidebar(false) } className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : 'hover:bg-gray-100' }`} >
                        {({isActive})=>(
                            <>
        ...
        <div onClick={openUserProfile} className='flex gap-2 items-center cursor-pointer'>
            ...
            <div>
               ...
               <p className='text-xs text-gray-500'>
                <Protect plan='premium' fallback="Free" >Preminm</Protect>Plan
               </p>
            </div>
        </div>
